...
import { register, setup, type Context } from './helpers';
...
/** Inscrit un compte sur l'agent principal et lui donne le rôle administrateur. */
async function loginAsAdmin(ctx: Context) {
  const account = await register(ctx.agent);
  await ctx.users.update(account.user.id, { role: 'admin' });
}
...
describe('accès au back-office (EF-7.5)', () => {
  it('est réservé au rôle administrateur', async () => {
    const ctx = await setup([]);
    await ctx.agent.get('/api/admin/categories').expect(401);
    expect((await ctx.agent.get('/api/admin/session').expect(200)).body).toEqual({ authenticated: false, admin: false });

    const player = await register(ctx.agent);
    await ctx.agent.get('/api/admin/categories').expect(403);
    expect((await ctx.agent.get('/api/admin/session').expect(200)).body).toEqual({ authenticated: true, admin: false });

    await ctx.users.update(player.user.id, { role: 'admin' });
    await ctx.agent.get('/api/admin/categories').expect(200);
    expect((await ctx.agent.get('/api/admin/session').expect(200)).body.admin).toBe(true);
  });
});
