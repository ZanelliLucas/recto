import { MISMATCH_DELAY_MS, type CardImage, type FinishGameResponse } from '@recto/shared';
...
function describe(event: FlipEvent | null, deck: readonly string[], images: ReadonlyMap<string, CardImage>): string {
  if (!event) return '';
  const title = (index: number | undefined) => images.get(deck[index ?? -1] ?? '')?.title ?? '';
...
function GameSession({ game, categoryName }: GameLocationState) {
  ...
  const images = useMemo(() => new Map(game.images.map((image) => [image.id, image])), [game]);
  ...
  useEffect(() => {
    starting.current ??= (async () => {
      preloaded.current = await preloadImages(
        game.images.map((image) => image.url),
        setLoaded,
      );
  ...
  const announcement = useMemo(() => describe(engine.lastEvent, engine.deck, images), [engine.lastEvent, engine.deck, images]);
    ...
    <div className={styles.session}>
      ...
      <div className={styles.stage}>
        <Board
          ...
          images={images}
          difficulty={game.difficulty}
          showImages={engine.phase !== 'loading'}
          onFlip={flip}
        />
