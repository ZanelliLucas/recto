...
import { getLastPlayed, getRecord } from '../game/records';
...
export function HomePage() {
  ...
  const record = last ? getRecord(last.category, last.difficulty) : undefined;
      ...
      <section className={styles.hero}>
        ...
        {last && (
          <div className={styles.replay}>
            ...
            {record && (
              <span className={styles.replayRecord}>{t('difficulty.record', { time: formatDuration(record.bestMs) })}</span>
            )}
          </div>
        )}
        {error && (
